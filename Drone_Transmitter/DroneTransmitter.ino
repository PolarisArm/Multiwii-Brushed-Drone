...
int PITCH_CENTER = 0, ROLL_CENTER = 0, YAW_CENTER = 0, THROTTLE_CENTER = 0;
...
void calibrate(){
  long t = 0, y = 0, r = 0, p = 0;

  for(int i=0;i<100;i++){
    y += map(analogRead(YAW),1023,14,0,255);
    r += map(analogRead(ROLL),0,1023,0,255);
    p += map(analogRead(PITCH),0,1023,0,255);
    t += map(analogRead(THROTTLE),1023,0,0,255);
    delay(5);
  }

  THROTTLE_CENTER = t / 100;
  YAW_CENTER = y/100;
  PITCH_CENTER = p / 100;
  ROLL_CENTER = r / 100;
...
int applyDeadband(int value, int center, int deadband = 5){
   value += (127 - center);
...
void send_data()
{
  data.throttle = map(analogRead(THROTTLE),1023,0,0,255);
  data.yaw      = applyDeadband(map(analogRead(YAW)     ,1023,14,0,255),YAW_CENTER,5);
  data.roll     = applyDeadband(map(analogRead(ROLL)    ,0,1023,0,255),ROLL_CENTER,5);
  data.pitch    = applyDeadband(map(analogRead(PITCH)   ,0,1023,0,255),PITCH_CENTER,5);
